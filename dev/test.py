...
class TreeLSTMCell(nn.Module):
    ...
    def reduce_func(self, nodes):
        ...
        # # Step 2
        h_iou = self.U_iou(th.cat((ht_0, ht_1), dim=1))    # (Nt, 3H)
        ...
        # Steps 3.c,d
        X = self.W_f(nodes.data["emb"])             # (Nt, Nchn, H)
        f_tk = th.sigmoid(X + f_cell + self.b_f)
        c_cell = th.sum(f_tk * c_child, dim=1)      # (Nt, H)

        return {"h": h_iou, "c": c_cell}

    def apply_node_func(self, nodes):
        iou = self.W_iou(nodes.data['emb']) + self.b_iou  # (nodes_num x 3*h_size)
        i, o, u = th.chunk(iou, 3, 1)  # (nodes_num x h_size) for each of i,o,u
        i, o, u = th.sigmoid(i), th.sigmoid(o), th.tanh(u)

        # Step:3
        # f = th.sigmoid(self.U_f(th.cat((h_0, h_1), dim=1)))  # (1 x 2*h_size)
        # f = f.view(2, -1)                                    # (2 x h_size  )
        # c_cell = th.sum(f * th.cat((c_0, c_1), dim=0), dim=0)   # (1 x h_size  )
        # c_cell = i * u + c_cell                          # (nodes_num  x h_size)

        # Step: 4
        h_cell = o      # (nodes_num  x h_size)
        return {"h": h_cell}
